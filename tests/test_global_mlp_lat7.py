...
from src.models.training.run_global_mlp_lat7 import (
    FEATURE_COLUMNS,
    standardize_cached_splits,
)


class GlobalMlpLat7Test(unittest.TestCase):
    def test_scaler_only_fits_train_and_preserves_feature_order(self):
        ...
        raw_splits = {
            "train": {
                "core6": core_train,
                "sin_latitude": np.asarray([-0.5, 0.0, 0.5], np.float32),
                "target": np.zeros((3, 2), np.float32),
            },
            "val": {
                "core6": np.asarray([[9, 10, 11, 12, 0, -1]], np.float32),
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            self.assertGreater(splits["X_val"][0, 0], 2.0)
            self.assertEqual(splits["feature_cols"], FEATURE_COLUMNS)
            self.assertEqual(FEATURE_COLUMNS[-1], "sin_latitude")
