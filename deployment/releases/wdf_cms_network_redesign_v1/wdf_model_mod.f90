...
!!     input_features 声明为 (6, N)     → C++ 视为 row-major (N, 6) ✓
...
!!     ...
!!     features(6, i) = era5_wave_dir_cos ← 粒子 i 的第 6 个特征
!!
!!   6 个特征的顺序（必须与 Python 训练时一致）：
!!     1: era5_u10           东向 10m 风速 (m/s)
!!     2: era5_v10           北向 10m 风速 (m/s)
!!     3: era5_swh           有效波高 (m)
!!     4: era5_mwp           平均波周期 (s)
!!     5: era5_wave_dir_sin  sin(波向)
!!     6: era5_wave_dir_cos  cos(波向)
!!
!!   输出 (2, N)：
...
contains
    ...
    !> 批量预测漂移残差
    !!
    !! @param features   输入特征数组，shape = (6, num_particles)
    !!                   ★ 第一维是特征，第二维是粒子编号
    !! @param np         粒子数量 N
    ...
    subroutine wdf_predict(features, np, drift_uv)
        ...
        real(c_float), intent(in)      :: features(6, np)
